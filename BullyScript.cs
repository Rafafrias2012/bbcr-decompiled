...
public class BullyScript : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		RaycastHit hitInfo;
		if (Physics.Raycast(base.transform.position + new Vector3(0f, 4f, 0f), direction, out hitInfo, float.PositiveInfinity, 769, QueryTriggerInteraction.Ignore) & (hitInfo.transform.tag == "Player") & ((base.transform.position - player.position).magnitude <= 30f) & active && !gc.NametagActive)
		{
			if (!spoken)
			{
